...
Pre-Vibe 是一个 Claude Code 插件，用来在新会话真正开始执行前，先把用户粗略的第一句话整理成项目专属启动上下文、关键问题、CLAUDE.md 兼容指引和紧凑的执行提示。

它面向刚开始使用 Claude Code 的用户、初级 builder，以及需求还短小、模糊、缺少执行细节的新项目会话。Pre-Vibe 会读取安全范围内的项目上下文，检查 Claude Code 指引，只询问真正阻塞的问题，写入项目启动文档，并在用户确认后通过 `FIRST_PROMPT.md` 交接。
...
安装后开启新的 Claude Code 会话，启用 **Pre-Vibe**。

## 快速开始

1. 在需要整理上下文的项目目录打开 Claude Code。
2. 从插件选框启用 **Pre-Vibe**。
3. 输入一句粗略需求，例如"帮我把这个想法整理成可执行开发计划"。
4. 如果 Claude Code 打开原生问题弹窗，直接在弹窗中回答。
5. 查看生成的项目启动文档。
6. 确认交接后，让 Claude Code 从 `FIRST_PROMPT.md` 继续执行。
...
- `CLAUDE.md`：当项目没有根级 agent 指引时创建。
- `PROJECT_CLAUDE.md`：当项目已有根级 `CLAUDE.md` 时，作为可审校的合并建议创建。
- `FIRST_PROMPT.md`：给 Claude Code 的紧凑执行契约，每次交接都会完整重写。
- `PROJECT_INDEX.md`：仅 architect 档生成，用于索引项目意图、资源、工具、文件、环境和目的。

`PRE_VIBE_SPEC.md`、agent 指引文档和 `PROJECT_INDEX.md` 保持职责独立。`FIRST_PROMPT.md` 可以引用 Claude Code 接手时必须读取的文件。
...
生成文档只是准备阶段，不是完成状态。只有用户确认交接并且 Claude Code 从 `FIRST_PROMPT.md` 继续执行，或者用户明确取消，本轮 Pre-Vibe 才算结束。
...
MCP server 只向用户展示短状态，把详细工作流数据交给 Claude Code。Python 层负责确定性路由、校验、扫描、设置和安全写入；最终任务专属 Markdown 由 Claude Code 根据用户输入和项目证据写作。
...
Pre-Vibe 使用 allowlist 项目扫描，跳过疑似敏感文件，把输出路径限制在当前项目内，并避免静默覆盖已有根级 `CLAUDE.md`。
